...
prompt = (
    ...
    f"Use the tool `get_article` to read in at least 3 Wikipedia page. "
    f"Only include content that you can find from the `get_article` tool. "
    f"Only reply with the podcast script. Don't anything like [host]: or [music fades in]. "
)

async def main():
    agent = Agent(config.model_name, toolsets=[server])
